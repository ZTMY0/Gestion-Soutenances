...
$stmtP = $pdo->prepare("SELECT * FROM projets WHERE etudiant_id = ? LIMIT 1");
$stmtP->execute([$etudiant_id]);
$projet = $stmtP->fetch(PDO::FETCH_ASSOC);

// Si pas de projet ou pas d'encadrant -> retour dashboard
if (!$projet || empty($projet['encadrant_id'])) {
    header("Location: index.php");
    exit();
}

// 3) CHECK DOUBLON (un seul check)
$stmtCheck = $pdo->prepare("SELECT COUNT(*) FROM rapports WHERE projet_id = ?");
$stmtCheck->execute([(int)$projet['id']]);
$dejaSoumis = (int)$stmtCheck->fetchColumn();
...
if (isset($_POST['upload_btn'])) {
    ...
    elseif (!isset($_FILES['rapport']) || $_FILES['rapport']['error'] !== 0) {
        ...
        $mots_cles = trim($_POST['mots_cles_rapport'] ?? '');
        $remerciements = trim($_POST['remerciements'] ?? '');

        $maxSize = 50 * 1024 * 1024; // 50 Mo
        $allowedExt = ['pdf'];
        $fileExt = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));

        // Vérif MIME (anti faux PDF)
        ...
        if ($resume === '') {
            ...
            $error = "Le fichier n'est pas un PDF valide (MIME incorrect).";
        } else {
            ...
            // Nom unique serveur
            $fileName = "rapport_" . (int)$projet['id'] . "_" . time() . ".pdf";
            $destPath = $uploadDir . $fileName;

            if (move_uploaded_file($file['tmp_name'], $destPath)) {

                // Insertion BDD (si ta table rapports n'a pas mots_cles/remerciements, on les ignore)
                ...
                $pdo->prepare("UPDATE projets SET statut = 'rapport_soumis' WHERE id = ?")
                    ->execute([(int)$projet['id']]);
...
<head>
    ...
    <style>
        .upload-area { border: 2px dashed #ced4da; border-radius: 10px; transition: all 0.3s ease; background: #f8f9fa; }
        .upload-area:hover, .upload-area.dragover { border-color: #0d6efd; background-color: #e9ecef; cursor: pointer; }
