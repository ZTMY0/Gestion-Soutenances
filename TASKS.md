...
Fichier : src/views/directeur/index.php
- Visualisation des PFE par filière (Chart.js)
- KPI : rapports déposés vs attendus
- Alertes :
  - Projets sans encadrant
  - Étudiants sans dépôt à J-7
- Comparaison des moyennes des notes (3 dernières années)

### 2. Validation stratégique (Planning)
Fichier : src/views/directeur/validation.php
- Vue globale du planning des soutenances
- Bouton : Approuver tout le planning (planifié → confirmé)
- Option : Demander correction au coordinateur
- Vérification surcharge des jurys

### 3. Signature électronique des PV
Fichier : src/views/directeur/signatures.php
- Liste des PV générés
- Bouton : Signer numériquement (simulation SHA-256)
- Changement statut : pv_genere → pv_signe
- Archivage du PDF signé

### 4. Gestion des comptes & paramètres
Fichier : src/views/directeur/parametres.php
- Activation / désactivation des coordinateurs
- Configuration règles métier :
  - Durée standard soutenance
